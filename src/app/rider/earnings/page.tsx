...
export default function RiderEarnings() {
 ...
 <p className="text-[10px] font-black text-zinc-400 tracking-widest mt-1">WALLET SUMMARY</p>
 </div>
 ...
 <span className="text-[10px] font-black text-primary tracking-widest block mb-2">Total Balance</span>
 <h3 className="text-5xl font-headline font-black tracking-tighter">₹{totalEarnings.toFixed(2)}</h3>
 <div className="mt-8 pt-8 border-t border-white/10 flex justify-between items-center">
 <div>
 <p className="text-[10px] font-black text-zinc-500 tracking-widest">Completed Jobs</p>
 <p className="text-xl font-headline font-black tracking-widest">{completedOrders.length}</p>
 </div>
 <div className="bg-white/10 text-white px-4 py-2 rounded-xl font-black text-[9px] tracking-widest border border-white/20">Cash Only</div>
 </div>
 </div>
 </section>

 <section className="space-y-4">
 <h3 className="text-[10px] font-black text-zinc-400 tracking-widest px-2">Job History</h3>
 {completedOrders.length === 0 ? (
 <p className="text-center py-10 text-zinc-400 text-xs font-bold font-headline tracking-widest">No payout history yet</p>
 ) : (
 ...
 <div>
 <p className="text-xs font-black text-zinc-900 tracking-tighter leading-none">#{order.id?.slice(-8).toUpperCase()}</p>
 <p className="text-[9px] font-bold text-zinc-400 tracking-widest mt-1">{new Date(order.createdAt).toLocaleDateString()}</p>
 </div>
 </div>
 <div className="text-right">
 <p className="text-sm font-headline font-black text-zinc-900 leading-none">+₹{(order.deliveryCharge || 30).toFixed(0)}</p>
 <p className="text-[8px] font-black text-zinc-400 tracking-widest mt-1">Delivery Fee</p>
 </div>
 </div>
 ))
 )}
