 ...
 <span className="material-symbols-outlined font-black">favorite</span>
  ...
  <div className="mt-6 flex flex-col gap-1">
    <div className="flex items-center justify-between">
      ...
      {/* Add to Cart Button (Moved under price) */}
      <div className="w-32">
        {outOfStock ? (
          <button disabled className="w-full h-10 bg-red-50 text-red-600 border border-red-100 rounded-xl font-black text-[10px] tracking-widest cursor-not-allowed uppercase">
            Sold Out
          </button>
        ) : !cartItem ? (
          <button onClick={() => addToCart({...product, quantity: 1})} className="w-full h-10 bg-primary text-zinc-900 rounded-xl font-black text-[10px] tracking-widest shadow-md shadow-primary/20 active:scale-95 transition-all uppercase">
            Add to cart
          </button>
        ) : (
          <div className="w-full h-10 bg-zinc-900 text-white rounded-xl flex items-center justify-between px-1 overflow-hidden shadow-md">
