...
function LoginContent() {
  ...
  useEffect(() => {
    if (isInitialized && user && userData) {
      // Only trigger if onboarding was NEVER completed
      const needsOnboarding = !userData.onboardingComplete || !userData.phoneNumber || !userData.address;
  ...
  return (
    <div className="min-h-screen bg-emerald-50/30 flex flex-col justify-center items-center py-12 px-6 relative overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        {step === 0 && (
          ...
          >
            ...
            <div className="bg-white p-8 md:p-10 rounded-[48px] shadow-2xl shadow-emerald-900/5 border border-white">
              {!showEmailLogin ? (
                <>
                  <div className="text-center mb-8">
                    <h2 className="text-xl font-bold text-zinc-900 mb-2">Welcome Back!</h2>
                    <p className="text-xs text-zinc-500 leading-relaxed">Fast, fresh groceries are just a tap away.</p>
                  </div>
                  ...
                  <div className="mt-6 flex flex-col items-center gap-4">
                    <div className="w-full h-px bg-zinc-100 relative">
                      <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-3 text-[10px] font-bold text-zinc-300 uppercase">Or use email</span>
                    </div>
                    <button 
                      onClick={() => setShowEmailLogin(true)}
                      className="text-xs font-black text-emerald-600 hover:text-emerald-700 tracking-widest uppercase transition-colors"
                    >
                      Sign in with Email
                    </button>
                  </div>
                ...
                <form onSubmit={handleEmailLogin} className="space-y-4">
                  <div className="mb-6">
                    <h2 className="text-xl font-bold text-zinc-900 mb-1">Email Login</h2>
                    <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Welcome Back</p>
                  </div>
                  <input 
                    type="email" 
                    required
                    ...
                    className="w-full px-6 py-4 bg-zinc-50 border border-zinc-100 rounded-2xl text-zinc-900 font-bold placeholder:text-zinc-300 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all text-sm"
                  />
                  <input 
                    type="password" 
                    required
                  ...
                  <button 
                    type="submit"
                    disabled={loading}
                    className="w-full h-14 bg-zinc-900 text-white rounded-2xl font-black text-xs tracking-widest uppercase shadow-xl shadow-zinc-200 active:scale-95 transition-all flex items-center justify-center"
                  >
                  ...
                  <button 
                    ...
                    className="w-full text-center text-[10px] font-black text-zinc-400 uppercase tracking-widest py-2"
                  >
        ...
        {step === 2 && (
          ...
          >
            <div className="bg-white p-10 rounded-[48px] shadow-2xl border border-white">
              ...
              />
              <button 
                onClick={() => validatePhone(onboardingData.phoneNumber) ? setStep(3) : toast.error("Please enter a valid 10-digit phone number")}
                className="w-full h-14 bg-emerald-500 text-white rounded-2xl font-black text-xs tracking-widest uppercase shadow-xl shadow-emerald-200 active:scale-95 transition-all"
              >
                Next
              </button>
      ...
      <p className="mt-10 text-center text-[9px] text-zinc-300 font-bold tracking-[0.2em] leading-relaxed uppercase z-10">
        BazaarBolt <span className="text-zinc-400">Secure Authentication</span>
      </p>
    </div>
  );
}
