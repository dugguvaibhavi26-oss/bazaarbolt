          ...
          <div className="fixed bottom-0 left-0 w-full bg-white border-t border-zinc-100 p-4 z-[60] shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
            <div className="max-w-3xl mx-auto flex items-center justify-between gap-4">
              ...
              <button onClick={handleCheckout} disabled={placingOrder} className="flex-1 bg-green-600 text-white h-14 rounded-xl flex items-center justify-between px-6 active:scale-[0.98] transition-all disabled:opacity-50">
                <div className="flex flex-col items-start leading-none uppercase"><span className="text-[14px] font-black">₹{total.toFixed(0)}</span><span className="text-[8px] font-bold tracking-widest opacity-80">Total</span></div>
                <div className="flex items-center gap-2"><span className="text-[14px] font-black tracking-widest uppercase">{placingOrder ? 'Wait...' : 'Place order'}</span><span className="material-symbols-outlined text-sm">arrow_right</span></div>
              </button>
            </div>
          </div>
        </>
