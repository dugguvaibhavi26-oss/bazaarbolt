...
export default function OrderTracking({ params }: { params: Promise<{ id: string }> }) {
 ...
 if (newlyUnavailable) {
 import("react-hot-toast").then(t => t.default.error(`Item Update: ${newlyUnavailable.name} is out of stock. Order total updated.`, {
 duration: 6000,
 icon: '⚠️'
 })
 );
 ...
 <h3 className="font-headline font-black text-xs tracking-widest text-[#1A1A1A] mb-6 flex items-center gap-2">
 ...
 <p className="text-[10px] font-bold text-zinc-400 tracking-widest">Qty: {item.quantity}</p>
 </div>
 ...
 {item.unavailable && <span className="text-[8px] font-black text-red-500 ">Unavailable</span>}
 </div>
 </div>
 ))}
 </div>
 ...
 <span className="text-xs font-bold text-zinc-400 tracking-widest">Subtotal</span>
 <span className="text-sm font-black text-zinc-600">₹{order.subtotal?.toFixed(2) || (order.total - (order.tax || 0)).toFixed(2)}</span>
 </div>
 <div className="flex justify-between items-center text-[#1A1A1A]">
 <span className="text-xs font-bold text-zinc-400 tracking-widest">Taxes & Fees</span>
 ...
 <span className="text-sm font-headline font-black text-zinc-900 tracking-tighter">Total Amount</span>
 <span className="text-[9px] font-bold text-primary tracking-widest">Via {order.paymentMethod === 'COD'? 'Cash': 'Online'}</span>
 </div>
 ...
 <div key={idx} className="flex justify-between items-center text-[10px] font-black text-red-400 ">
 <span>{item.name}</span>
 ...
 <span className="font-headline text-[9px] font-black tracking-widest mt-1">Home</span>
 </button>
 ...
 <span className="font-headline text-[9px] font-black tracking-widest mt-1 text-zinc-900">Orders</span>
 </button>
 ...
 <span className="font-headline text-[9px] font-black tracking-widest mt-1">Helpdesk</span>
 </button>
 ...
 <span className="font-headline text-[9px] font-black tracking-widest mt-1">Profile</span>
 </button>
 </nav>
 </>
 );
