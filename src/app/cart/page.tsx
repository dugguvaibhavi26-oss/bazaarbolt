...
export default function CartPage() {
 ...
 const displayAddress = selectedAddress ?`${selectedAddress.line1}, ${selectedAddress.city}`: "Select Delivery Address";
 ...
 <span className="font-headline font-black text-primary bg-primary/10 px-2 py-0.5 rounded-md text-[10px] tracking-widest ">{cartCount} ITEMS</span>
 </div>
 ...
 <Link href="/"className="inline-flex items-center text-zinc-900 font-headline font-black text-xs tracking-widest bg-primary px-8 py-4 rounded-2xl shadow-lg active:scale-95 transition-transform">
 Explore Store
 </Link>
 </div>
 ) : (
 ...
 <p className="text-zinc-400 text-[8px] font-bold tracking-widest mb-1.5">{item.category}</p>
 ...
 <span className="font-black text-[9px] tracking-widest text-zinc-900">'{activeCoupon.code}'Matched</span>
 <span className="text-[7px] text-zinc-500 font-bold tracking-wider">Saving ₹{discountAmount.toFixed(0)}</span>
 </div>
 </div>
 <button onClick={removeCoupon} className="bg-white px-2 py-1 rounded text-red-500 font-black text-[8px] tracking-widest border border-red-50 active:scale-95">Cancel</button>
 </div>
 ) : showCouponInput ? (
 ...
 autoFocus
 />
 <button type="submit"className="bg-zinc-900 text-white font-black text-[8px] px-4 py-2 rounded tracking-widest active:scale-95 transition-transform">Apply</button>
 </form>
 ) : (
 ...
 <h3 className="font-black text-[8px] tracking-[0.2em] text-zinc-400 ml-1">Billing Summary</h3>
 ...
 <span className="text-primary font-black text-[8px] tracking-widest">{deliveryFee === 0 ? 'ZERO FEE':`₹${deliveryFee}`}</span>
 </div>
 </div>
 ...
 <button onClick={handleProceed} className="bg-zinc-900 text-white font-black text-[10px] tracking-widest px-6 py-3 rounded-xl shadow-lg active:scale-95 transition-transform flex items-center gap-2">
 Next Step
