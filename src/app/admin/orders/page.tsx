    ...
    <div className="space-y-8 pb-32">
       <div className="flex items-center justify-between">
          ...
          <div className="flex bg-white p-1 rounded-2xl border border-zinc-200 shadow-sm">
             {["ALL", "PLACED", "PICKED", "DELIVERED"].map(f => (
                <button 
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-primary text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-900'}`}
                >
                   {f}
                </button>
             ))}
          ...
          <div className="overflow-x-auto">
             <table className="w-full text-left border-collapse">
                <thead>
                   <tr className="bg-zinc-50 text-zinc-400">
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest">Order / Customer</th>
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest">Items & Bag</th>
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest">Pricing</th>
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-center">Lifecycle Status</th>
                   ...
                   {filteredOrders.map(o => (
                      <tr key={o.id} className="hover:bg-zinc-50/50 transition-colors group">
                         <td className="px-8 py-6">
                            <div className="flex items-center gap-4">
                               ...
                               <div className="flex flex-col">
                                  <span className="font-headline font-black text-xs text-zinc-900 uppercase tracking-tighter">#{o.id.slice(-8).toUpperCase()}</span>
                         ...
                         <td className="px-8 py-6">
                            ...
                            <p className="text-[9px] font-bold text-zinc-400 mt-1 uppercase tracking-widest">{o.paymentMethod || 'COD'}</p>
                         </td>
                         <td className="px-8 py-6">
                            <div className="flex justify-center">
                               <span className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm
                                 ${o.status === 'DELIVERED' ? 'bg-green-100 text-green-700' : 
                                   o.status === 'PICKED' ? 'bg-orange-100 text-orange-700' : 
             ...
             {filteredOrders.length === 0 && (
                <div className="p-20 text-center">
                   ...
                   <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">No Orders in this queue</p>
                </div>
             )}
          </div>
       </div>
