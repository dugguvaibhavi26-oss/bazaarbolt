       ...
       <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 bg-white rounded-[40px] p-10 border border-zinc-100 shadow-sm relative overflow-hidden">
             <div className="flex items-center justify-between mb-8">
                <h3 className="font-headline font-black text-xl text-zinc-900 uppercase tracking-tight">Rapid Order Feed</h3>
                <Link href="/admin/orders" className="text-[10px] font-black uppercase text-primary bg-zinc-900 px-4 py-2 rounded-xl tracking-widest">Full History</Link>
             </div>
                ...
                {recentOrders.map(order => (
                   <div key={order.id} className="flex items-center justify-between p-6 bg-zinc-50 hover:bg-zinc-100 transition-colors rounded-[28px] group">
                      <div className="flex items-center gap-5">
                         ...
                         <div>
                            <p className="font-headline font-black text-sm text-zinc-900 uppercase tracking-tight">#{order.id?.slice(-8).toUpperCase()}</p>
                            <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-1">₹{order.total.toFixed(2)} • {order.items.length} Items</p>
                         </div>
                      </div>
                      <div className="flex items-center gap-4">
                         <span className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm
                           ${order.status === 'DELIVERED' ? 'bg-green-100 text-green-700' : 
                             order.status === 'CANCELLED' ? 'bg-red-100 text-red-700' : 'bg-white text-zinc-900'}
                         `}>
                           {order.status}
                         </span>
          ...
          <div className="lg:col-span-4 space-y-8">
             <div className="bg-zinc-900 rounded-[40px] p-8 text-white shadow-xl relative overflow-hidden">
                ...
                <div className="space-y-6 relative z-10">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                         <p className="text-xs font-black uppercase tracking-widest">Store Gateway Open</p>
                         <p className="text-[9px] font-bold text-zinc-500 mt-1 uppercase tracking-widest">Processing Instant Orders</p>
                      </div>
                   </div>
                   <div className="pt-6 border-t border-white/10">
                      <div className="flex items-center justify-between mb-4">
                         <span className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Active Riders</span>
                         <span className="text-xs font-black text-primary">12 Ready</span>
                      </div>
                      <div className="flex items-center justify-between">
                         <span className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Avg Pulse Delivery</span>
             ...
             <div className="bg-white rounded-[40px] p-8 border border-zinc-100 shadow-sm">
                <h4 className="font-headline font-black text-[10px] uppercase tracking-widest text-zinc-400 mb-6">Stock Alerts</h4>
                <div className="space-y-4">
                   <div className="flex items-center justify-between p-4 bg-red-50 rounded-2xl border border-red-100">
                      <span className="text-[10px] font-black text-red-600 uppercase tracking-widest">Amul Gold Milk</span>
                      <span className="text-[10px] font-black text-red-400">Low Stock</span>
                   </div>
                   <div className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl">
                      <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Organic Avocado</span>
