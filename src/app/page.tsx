      ...
      <div className={`flex flex-col gap-1 transition-all group ${outOfStock ? 'opacity-60 grayscale' : ''}`}>
        ...
        <div className="flex flex-col px-0.5 mt-0.5">
          <div className="flex items-center gap-1 mb-0.5 h-3">
            ...
            {product.stock < 10 && product.stock > 0 && <span className="text-orange-600 text-[7px] font-bold tracking-tight whitespace-nowrap">Low Stock</span>}
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-[7px] text-zinc-400 line-through font-medium tracking-tight opacity-50">₹{(product.price * 1.25).toFixed(0)}</span>
          </div>
        </div>
      </div>
    );
