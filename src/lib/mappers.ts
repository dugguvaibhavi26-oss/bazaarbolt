...
export const mapProduct = (doc: DocumentSnapshot<DocumentData>): Product => {
  ...
  return {
    ...
    lastUpdatedBy: typeof data.lastUpdatedBy === "string" ? data.lastUpdatedBy : null,
...
export const mapOrder = (doc: DocumentSnapshot<DocumentData>): Order => {
  ...
  const items = Array.isArray(data.items) ? data.items.map((item: any, idx: number): CartItem => {
    return {
      ...
      unavailableAt: typeof item.unavailableAt === "string" ? item.unavailableAt : null,
      replacedBy: typeof item.replacedBy === "string" ? item.replacedBy : null,
      vendorStatus: ["PENDING", "ACCEPTED", "REJECTED"].includes(item.vendorStatus) ? item.vendorStatus : "PENDING",
      vendorId: typeof item.vendorId === "string" ? item.vendorId : null,
    };
  }) : [];
  ...
  return {
    ...
    rated: typeof data.rated === "boolean" ? data.rated : null,
  };
};
